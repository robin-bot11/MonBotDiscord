            ...
            class AcceptButton(discord.ui.View):
                ...
                async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
                    try:
                        ...
                        await interaction.response.send_message("❌ Impossible d'ajouter le rôle. Vérifiez les permissions.", ephemeral=True)

            view = AcceptButton(data.get("role"), data.get("button"), data.get("emoji"))
            await ctx.send(embed=embed, view=view)

        except Exception as e:
            await ctx.send(f"❌ Impossible d'afficher le règlement : {e}")
